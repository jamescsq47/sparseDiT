    ...
    for step in range(comm.world_size):
        ...
        if not causal or step <= comm.rank:
            ...
            if attn_type == AttnType.SPARGE:
                import torch.distributed as dist
                dist.barrier()  # 所有进程同步到这里
                if dist.get_rank() == 0:
                    import ipdb; ipdb.set_trace()
                dist.barrier()  # 断点后再同步一次
                out, head_density = fn(
                    q,
                    k,
                    v,
...
class RingFlashAttnFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.sparse = sparse
        return (out, head_density) if attn_type == AttnType.SPARGE else out if not return_softmax else (out, softmax_lse, None)
