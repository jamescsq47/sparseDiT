        ...
        *args: Any,
        ...
        if self.use_pack_qkv:
            ...
            if transpose_matrix_q is not None and transpose_matrix_q_T is not None:
                ...
                query_layer=query_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_row_perm_idx[0:seqlen//64]).contiguous()
                print(f"rank {dist.get_rank()}, before all2all q: {query_layer.shape},{transpose_matrix_q.shape},{transpose_matrix_q_T.shape}")
                dist.all_to_all_single(query_layer,query_layer,transpose_matrix_q_T.tolist(),transpose_matrix_q.tolist(), group=self.ring_pg)
            ...
            if transpose_matrix_k is not None and transpose_matrix_k_T is not None:
                batch_size, seqlen, nheads, d = key_layer.shape
                key_layer=key_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_col_perm_idx[0:seqlen//64]).contiguous()
                dist.all_to_all_single(key_layer,key_layer,transpose_matrix_k_T.tolist(),transpose_matrix_k.tolist(), group=self.ring_pg)
                key_layer=key_layer.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous().index_select(0, new_row_deperm_idx[0:seqlen//64]).contiguous()

                value_layer=value_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1)
                dist.all_to_all_single(value_layer,value_layer,transpose_matrix_k_T.tolist(),transpose_matrix_k.tolist(), group=self.ring_pg)
        ...
        if transpose_matrix_o is not None and transpose_matrix_o_T is not None:
            ...
            context_layer=context_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_row_deperm_idx[0:seqlen//64]).contiguous()
            ...
            context_layer=context_layer.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous()
