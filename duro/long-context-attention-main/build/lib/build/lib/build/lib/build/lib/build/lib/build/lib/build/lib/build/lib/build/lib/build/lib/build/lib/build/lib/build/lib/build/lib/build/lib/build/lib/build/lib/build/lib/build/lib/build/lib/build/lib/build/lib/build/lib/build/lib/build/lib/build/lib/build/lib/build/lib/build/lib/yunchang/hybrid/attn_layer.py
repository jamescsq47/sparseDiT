        ...
        *args: Any,
        ...
        if self.use_pack_qkv:
            ...
            if transpose_matrix_q is not None and transpose_matrix_q_T is not None and transpose_matrix_k is not None and transpose_matrix_k_T is not None:
                ...
                query_layer=query_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_row_perm_idx).contiguous()
                with torch.cuda.stream(comm_stream):
                    query_layer_perm = torch.empty_like(query_layer)
                    dist.all_to_all_single(query_layer_perm,query_layer,transpose_matrix_q_T.tolist(),transpose_matrix_q.tolist(), group=self.ring_pg, async_op=True)

                with torch.cuda.stream(compute_stream):
                    key_layer=key_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_col_perm_idx).contiguous()
                
                compute_stream.synchronize()

                with torch.cuda.stream(comm_stream):
                    key_layer_perm = torch.empty_like(key_layer)
                    dist.all_to_all_single(key_layer_perm,key_layer,transpose_matrix_k_T.tolist(),transpose_matrix_k.tolist(), group=self.ring_pg, async_op=True)

                with torch.cuda.stream(compute_stream):
                    value_layer=value_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_col_perm_idx).contiguous()

                compute_stream.synchronize()

                value_layer_perm = torch.empty_like(value_layer)
                dist.all_to_all_single(value_layer_perm,value_layer,transpose_matrix_k_T.tolist(),transpose_matrix_k.tolist(), group=self.ring_pg, async_op=True)
                    
                comm_stream.synchronize()
                query_layer=query_layer_perm.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous()
                key_layer=key_layer_perm.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous()
                value_layer=value_layer_perm.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous()
