...
def hybrid_permute_v3(
    ...
    if ring_degree == 1:
        row_perm_idx = torch.arange(H, device=sparse.device) 
        col_perm_idx = torch.arange(W, device=sparse.device)
        new_row_deperm_idx = torch.arange(H, device=sparse.device)
        new_col_deperm_idx = torch.arange(W, device=sparse.device)
        sparse_final = sparse_reordered
    else:
        assert H % ring_degree == 0 and W % ring_degree == 0, "H和W必须能被ring_degree整除"
...
if __name__ == "__main__":
    ...
    with profile(
        ...
        for _ in range(3):
            local_out = usp_attn(
                ...
                transpose_matrix_q = transpose_matrix_q[rank%sp_ring_degree].contiguous(),
                transpose_matrix_q_T = transpose_matrix_q.T[rank%sp_ring_degree].contiguous(),
                transpose_matrix_k = transpose_matrix_k[rank%sp_ring_degree].contiguous(),
                transpose_matrix_k_T = transpose_matrix_k.T[rank%sp_ring_degree].contiguous(),
                transpose_matrix_o = transpose_matrix_q.T[rank%sp_ring_degree].contiguous(),
                transpose_matrix_o_T = transpose_matrix_q[rank%sp_ring_degree].contiguous(),
            )
            torch.cuda.synchronize()
    ...
    if rank == 1 :
        ...
        prof.export_chrome_trace(f"profile/wan/profile_rank{rank}_ulysses{sp_ulysses_degree}ring{sp_ring_degree}_block{block}_overhead.json")  # 可选：导出火焰图
