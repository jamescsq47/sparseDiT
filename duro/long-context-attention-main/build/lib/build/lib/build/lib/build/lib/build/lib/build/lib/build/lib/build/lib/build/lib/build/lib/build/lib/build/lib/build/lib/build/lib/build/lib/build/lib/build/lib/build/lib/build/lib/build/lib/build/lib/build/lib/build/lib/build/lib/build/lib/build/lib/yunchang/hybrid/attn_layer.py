        ...
        if self.use_pack_qkv:
            ...
            if transpose_matrix_q is not None and transpose_matrix_q_T is not None and transpose_matrix_k is not None and transpose_matrix_k_T is not None:
                ...
                with torch.cuda.stream(compute_stream):
                    ...
                    compute_stream.record_event(event_q)

                with torch.cuda.stream(comm_stream):
                    comm_stream.wait_event(event_q)
                    query_comm = torch.empty_like(query_layer)
                    dist.all_to_all_single(query_comm, query_layer,transpose_matrix_q_T.tolist(), transpose_matrix_q.tolist(),group=self.ring_pg, async_op=True)

                # ---------- K ----------
                with torch.cuda.stream(compute_stream):
                    key_layer = key_layer.index_select(0, new_col_perm_idx).contiguous()
                    event_k = torch.cuda.Event()
                    compute_stream.record_event(event_k)

                with torch.cuda.stream(comm_stream):
                    comm_stream.wait_event(event_k)
                    key_comm = torch.empty_like(key_layer)
                    dist.all_to_all_single(key_comm, key_layer,transpose_matrix_k_T.tolist(), transpose_matrix_k.tolist(),group=self.ring_pg, async_op=True)

                # ---------- V ----------
                with torch.cuda.stream(compute_stream):
                    value_layer = value_layer.index_select(0, new_col_perm_idx).contiguous()
                    event_v = torch.cuda.Event()
                    compute_stream.record_event(event_v)

                with torch.cuda.stream(comm_stream):
                    comm_stream.wait_event(event_v)
                    value_comm = torch.empty_like(value_layer)
                    dist.all_to_all_single(value_comm, value_layer,transpose_matrix_k_T.tolist(), transpose_matrix_k.tolist(),group=self.ring_pg, async_op=True)

                # ---------- 等待所有通信完成 ----------
                comm_stream.synchronize()

                # ---------- reshape back ----------
                query_layer = query_comm.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous()
                key_layer   = key_comm.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous()
                value_layer = value_comm.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous()
