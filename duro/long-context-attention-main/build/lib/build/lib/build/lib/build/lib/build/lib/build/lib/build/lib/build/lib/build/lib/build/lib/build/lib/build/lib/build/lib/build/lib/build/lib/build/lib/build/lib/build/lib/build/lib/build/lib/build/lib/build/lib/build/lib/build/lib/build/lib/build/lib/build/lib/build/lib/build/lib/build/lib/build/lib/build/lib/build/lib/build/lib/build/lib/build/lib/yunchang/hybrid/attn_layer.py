        ...
        *args: Any,
        ...
        if self.use_pack_qkv:
            ...
            if transpose_matrix_k is not None and transpose_matrix_k_T is not None:
                ...
                value_layer=value_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_col_perm_idx[0:seqlen//64]).contiguous()
        ...
        print(f"context_layer:{context_layer.shape}, transpose_matrix_o:{None if transpose_matrix_o is None else transpose_matrix_o.shape}, new_row_deperm_idx:{None if new_row_deperm_idx is None else new_row_deperm_idx.shape}")
        if transpose_matrix_o is not None and transpose_matrix_o_T is not None:
            ...
            context_layer=context_layer.transpose(0,1).reshape(batch_size, seqlen, nheads, d).index_select(0, new_row_deperm_idx[0:seqlen//64]).contiguous()
