        ...
        *args: Any,
        ...
        if self.use_pack_qkv:
            ...
            if transpose_matrix_k is not None and transpose_matrix_k_T is not None:
                ...
                key_layer=key_layer.transpose(0,1).reshape(batch_size, seqlen, nheads, d).contiguous().index_select(1, new_row_deperm_idx).contiguous()
        ...
        if transpose_matrix_o is not None and transpose_matrix_o_T is not None:
            ...
            context_layer=context_layer.reshape(batch_size, seqlen//64, 64, nheads, d).transpose(0,1).index_select(0, new_row_deperm_idx).contiguous()
